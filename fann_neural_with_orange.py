...
"""
Coming from http://www.j2m.cz/~jm/orange_hacks/fann_neural.py
...
class FannNeuralNetPickable:
    ...
    def __setstate__(self, odict):
        fake_file_call_s2f( self.ann.create_from_file,
                            odict.pop('fann_save') )
...
class AutoScaler:
    ...
    def train(self, array):
        def get_trans_fc(MIN, MAX, a, b):
            def linear_scale(number):
                assert a <= b
                ...
                if MIN == MAX:
                    ...
                    return float(a + b) / 2
                return a + (number - MIN) * ( float(b - a) / (MAX -  MIN) )
        ...
        for col in xrange(cols):
            ...
            #self.MIN.append(mi)
            #self.MAX.append(ma)
            self.trans.append(get_trans_fc(mi, ma, self.a, self.b))
            self.trans_back.append(get_trans_fc(self.a, self.b, mi, ma))
...
def test_housing():
    ...
    print ["%.4f" % score for score in Orange.evaluation.scoring.RMSE(cv)]
